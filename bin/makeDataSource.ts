...
const allFiles = [ 
    ...parseFiles(`${POSTS_PATH}/**/*(*md|*pm6)`),  
...parseFiles(`${pathFs.join(process.cwd())}/src/site/**/*(*.pod6)`)]
...
allFiles.flat().map((record: pubRecord & {puburl:string}) => {
  // filling title
  let title
  ...
  const publishUrl =  conf.exists("puburl")
    ...
    : record.file.includes(POSTS_PATH) ? record.file.substring(POSTS_PATH.length): record.file
